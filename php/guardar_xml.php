...
function guardar_xml($id_oa){
	...
	$filename= "$fila_oas[titular]";
 	...
 	if (file_put_contents($filename ,$xml)){
		...
		$qry = "INSERT INTO docs(id, id_OA, documento, nombre) VALUES ('$id', '$id_oa', '$contents', '$filename')";

		$mysqli-> query($qry) or die ($mysqli->error. " en la línea ".(__LINE__-1));

		unlink($filename);
	}
	else{
		print "El fichero del Material no ha podido crearse. Ha habido un error";
	}
